...
【输出格式】
请严格按照以下结构输出分析结果，确保每个维度都有量化的分数（0-100）。

评分数据用纯文本表格（| 用 | 分隔列）来呈现，表格内容必须是基于采集数据的真实数值，不要给占位符或示例格式。每个表格前后用空行隔开。

每个维度下方单独展开分析，使用【】标记重点（如【是否有降价空间】）。每个评分点单独成行，段落之间用空行分隔。
...
重要：整个回答中不要使用 * 或 ** 或 # 等 markdown 标记符号，用【】和 emoji 来组织结构。
"""
